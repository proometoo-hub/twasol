...
router.post('/register', async (req, res) => {
  ...
  const existing = db.prepare(`SELECT id FROM users WHERE phone = ? OR email = ? OR username = ?`).get(valid.phone || null, valid.email || null, normalizedUsername);
...
router.get('/me', requireAuth, (req, res) => res.json({ user: sanitizeUser(req.user) }));
